...
import type { Room, Player, Round, Answer } from "@/lib/supabase/database.types";
...
export function useGameState(roomId: string | null) {
  ...
  return {
    ...
    loading:
      room.loading || players.loading || rounds.loading || answers.loading,
    error:
      room.error || players.error || rounds.error || answers.error || null,
    refetch: {
      room: room.refetch,
